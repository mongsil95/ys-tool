...
export default function RpmPeriodPage() {
  ...
  const handleRpmChange = useCallback(
    (val: string) => {
      ...
      if (!isNaN(n) && n > 0) {
        const t = calcRpmToPeriod(n, timeUnit);
        setPeriod(t !== null ? String(parseFloat(t.toFixed(6))) : "");
      } else {
        setPeriod("");
      }
    },
  ...
  const handlePeriodChange = useCallback(
    (val: string) => {
      ...
      if (!isNaN(t) && t > 0) {
        const n = calcPeriodToRpm(t, timeUnit);
        setRpm(n !== null ? String(parseFloat(n.toFixed(6))) : "");
      } else {
        setRpm("");
      }
    },
  ...
  const handleTimeUnitChange = useCallback(
    (newUnit: TimeUnit) => {
      ...
      if (!isNaN(t) && t > 0) {
        const converted = convertPeriod(t, timeUnit, newUnit);
        setPeriod(String(parseFloat(converted.toFixed(6))));
      }
    },
    [timeUnit, period]
  );
        ...
        >
          {(["ms", "s"] as TimeUnit[]).map((u) => (
            <button
              key={u}
              type="button"
              onClick={() => handleTimeUnitChange(u)}
